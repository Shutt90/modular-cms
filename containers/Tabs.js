    ...
    <>
        ...
        <div className={`container`} id={`${tabs[0]}`}>
            <form method="POST" className="edit-form" action={`/api/edit/page/${model.id}`}>
                {addInput('text', 'model', removedUndefined(model.title), model.id, 'Title')}
                {addInput('textarea', 'model', removedUndefined(model.content), model.id, 'Content')}
                {addInput('text','model', model.priority, model.id, 'Priority')}
                {addDropdown(active, 'active', model.id, model.active,  'Active?', true)}
                {addInput('text','model', removedUndefined(model.metatitle), model.id, 'Meta Title')}
                {addInput('textarea','model', removedUndefined(model.metadesc), model.id, 'Meta Description')}
                {addInput('text','model', removedUndefined(model.metakeywords), model.id, 'Meta Keywords')}
                {addSubmit()}
            </form>
        </div>
